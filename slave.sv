...
begin
      ...
      else if(busy == 1'b0)//A)it means if it is not received any transaction start counter from middle
       begin
       pulse <= 2;
       count1 <= 202;/////A) here
       end
...
if(rst)
 begin
                  ...
                  state  <= idle;
                  r_addr <= 7'b0000000;
                  sda_en <= 1'b0;// to manage its direction
                  sda_t <= 1'b0; //to have its value
                  addr  <= 0;
                  r_mem <= 0;
                  din   <= 8'h00; 
                  ack_err <= 0;
    ...
    begin
      case(state)
               idle: begin 
                   if(scl == 1'b1 && sda == 1'b0) /// captures start con
                    begin
                    busy <= 1'b1;
                    state <= wait_p; 
                    end
